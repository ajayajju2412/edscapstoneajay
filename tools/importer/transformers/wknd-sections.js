 ...
 * Driven by payload.template.sections from page-templates.json (homepage: 5
 * sections). Selectors come directly from each section's `selector` array
 * (DOM-verified during page analysis).
 *
 * Breaks are inserted in beforeTransform (while every section element still
 * exists, before block parsers replace them) using a marker <hr>; Section
 * Metadata blocks are inserted in afterTransform, anchored to that marker.
 * Sections are iterated in reverse so live element references stay stable.
 */
